...
public class DocumentFactory {
    ...
    public static Document newDocument(String title, DocType type, boolean isFree, Object... args) {
        switch (type) {
            case BOOK :
                return new Book(title, args[0].toString(), isFree ? new Free() : new Borrowed());
            case CD :
                return new CD(title, args[0].toString(), isFree ? new Free() : new Borrowed());
            default :
                return new DVD(title, args[0].toString(), isFree ? new Free() : new Borrowed());
        }
    }
}
